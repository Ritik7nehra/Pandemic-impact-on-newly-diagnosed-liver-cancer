...
OUTPUT_PATH = BASE_DIR / "reports" / "static_dashboard.html"
...
def style_fig(fig, title):
    fig.update_layout(
        ...
        margin=dict(l=25, r=25, t=75, b=45),
...
def percent_stacked(df, category, title):
    tmp = df.groupby(["Cohort", category], dropna=False).size().reset_index(name="Cases")
    ...
    fig = px.bar(
        ...
        y="Percent",
        color=category,
        text=tmp["Percent"].map(lambda x: f"{x:.0f}%"),
        color_discrete_sequence=COLOR_SEQUENCE,
        category_orders={"Cohort": COHORT_ORDER},
        hover_data={"Cases": True, "Percent": ":.1f"},
    )
    fig.update_layout(xaxis_title="", yaxis_title="Percent of cases")
...
def main():
    ...
    pre_n = int((df["Cohort"] == "Pre-pandemic").sum())
    pan_n = int((df["Cohort"] == "Pandemic").sum())
    case_delta = (pan_n - pre_n) / pre_n * 100
    ...
    fig_case_mix = px.bar(
        ...
        x="Cohort",
        y="Cases",
        color="Cancer_Type_Simple",
        barmode="group",
        text_auto=True,
        color_discrete_sequence=COLOR_SEQUENCE,
        category_orders={"Cohort": COHORT_ORDER, "Cancer_Type_Simple": ["HCC", "ICC", "Review"]},
    )
    ...
    monthly = df.groupby(["Cohort", "Month_Order", "Month_Name"], dropna=False).size().reset_index(name="Cases")
    monthly = monthly.sort_values(["Cohort", "Month_Order"])
    fig_monthly = px.line(
        monthly,
        x="Month_Name",
        y="Cases",
    ...
    fig_size = px.histogram(
        ...
        color_discrete_sequence=COLOR_SEQUENCE,
        category_orders={"Cohort": COHORT_ORDER},
        labels={"Size": "Tumour size (mm)"},
    )
    fig_size = style_fig(fig_size, "Tumour size distribution")

    hcc_df = df[df["Cancer_Type_Simple"] == "HCC"]
    icc_df = df[df["Cancer_Type_Simple"] == "ICC"]
    ...
    fig_missing = px.bar(
        ...
        text=missing["Missing_Percent"].map(lambda x: f"{x:.1f}%"),
    )
    fig_missing.update_yaxes(autorange="reversed")
    fig_missing.update_layout(xaxis_title="Missing values (%)", yaxis_title="", showlegend=False)
    fig_missing = style_fig(fig_missing, "Data quality: top missing fields")

    pre = df[df["Cohort"] == "Pre-pandemic"]
    pan = df[df["Cohort"] == "Pandemic"]
    sym_pre = pre["Mode_Presentation"].eq("Symptomatic").mean() * 100
    sym_pan = pan["Mode_Presentation"].eq("Symptomatic").mean() * 100
    surv_pre = pre["Mode_Presentation"].eq("Surveillance").mean() * 100
    surv_pan = pan["Mode_Presentation"].eq("Surveillance").mean() * 100
    med_surv_pre = pre["Survival_fromMDM"].median()
    med_surv_pan = pan["Survival_fromMDM"].median()
    ...
    <section class="insights">
        <h2>Key signals in this dataset</h2>
        <p><b>Case volume fell</b> from {pre_n:,} to {pan_n:,} records ({case_delta:.1f}%).</p>
        <p><b>Symptomatic presentation increased</b> from {sym_pre:.1f}% to {sym_pan:.1f}%, while surveillance presentation moved from {surv_pre:.1f}% to {surv_pan:.1f}%.</p>
        <p><b>Median survival from MDM</b> shifted from {med_surv_pre:.1f} to {med_surv_pan:.1f} months. Treat this as descriptive, not causal, because follow-up time and case mix may differ.</p>
    </section>
    """
    ...
    chart_html = "\n".join(f"<div class='chart'>{fig_html(fig)}</div>" for fig in charts if fig is not None)
...
<title>COVID-19 Liver Cancer Dashboard</title>
...
<style>
    ...
    * {{ box-sizing: border-box; }}
    body {{
        ...
        color: var(--text);
        background:
            radial-gradient(circle at 10% 4%, rgba(36,240,182,0.15), transparent 26%),
            radial-gradient(circle at 90% 16%, rgba(124,92,255,0.18), transparent 30%),
            linear-gradient(135deg, #08111f 0%, #0e1726 52%, #070b14 100%);
        min-height: 100vh;
    }}
    .wrap {{ width: min(1440px, 94vw); margin: 0 auto; padding: 34px 0 52px; }}
    .hero {{
        padding: 30px 34px;
        border: 1px solid var(--line);
        border-radius: 30px;
        background: linear-gradient(135deg, rgba(36,240,182,0.16), rgba(124,92,255,0.13));
        box-shadow: 0 28px 70px rgba(0,0,0,0.32);
    }}
    .eyebrow {{ color: var(--green); font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; font-size: 12px; }}
    h1 {{ font-size: clamp(38px, 5vw, 70px); line-height: 0.95; margin: 12px 0 14px; letter-spacing: -0.06em; }}
    .subtitle {{ color: var(--muted); max-width: 960px; font-size: 18px; line-height: 1.55; }}
    .metrics {{ display: grid; grid-template-columns: repeat(7, minmax(150px, 1fr)); gap: 14px; margin: 18px 0; }}
    .metric-card {{
    ...
    .metric-value {{ font-size: 29px; font-weight: 850; letter-spacing: -0.04em; margin-top: 5px; }}
    .metric-note {{ color: var(--green); font-size: 12px; margin-top: 4px; }}
    .insights {{
        padding: 20px 24px;
        margin: 18px 0;
    ...
    .insights h2 {{ margin: 0 0 8px; letter-spacing: -0.03em; }}
    .insights p {{ margin: 7px 0; line-height: 1.5; }}
    ...
    .note {{ color: var(--muted); margin-top: 22px; font-size: 13px; line-height: 1.5; }}
    @media (max-width: 1180px) {{ .metrics {{ grid-template-columns: repeat(3, 1fr); }} .grid {{ grid-template-columns: 1fr; }} }}
    @media (max-width: 720px) {{ .metrics {{ grid-template-columns: 1fr; }} .hero {{ padding: 24px; }} }}
</style>
</head>
<body>
<main class="wrap">
    <section class="hero">
        <div class="eyebrow">Primary liver cancer services dashboard</div>
        <h1>COVID-19 impact on newly diagnosed liver cancer</h1>
        <div class="subtitle">A descriptive dashboard comparing the 12 months before the pandemic with the first 12 months of the pandemic for referrals to the NUTH HPB multidisciplinary team.</div>
    </section>
    <section class="metrics">{cards}</section>
    {insight_html}
    <section class="grid">{chart_html}</section>
    <p class="note">Source: uploaded COVID liver cancer CSV. Dashboard fields are descriptive and should not be interpreted as causal clinical evidence. Missing data are retained and visible in the data-quality chart.</p>
</main>
</body>
</html>
"""
